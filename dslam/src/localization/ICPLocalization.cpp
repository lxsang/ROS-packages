...
bool ICPLocalization::__match(const void (*callback)(std::vector<Line>&, pcl::PointCloud<pcl::PointXYZ>& cloud))
{
    ...
    try
    {
        ...
        current_kf_.tf.rotation = (current_feature_.orientation*last_features_.orientation.inverse())*current_kf_.tf.rotation;
        ...
        if(icp->hasConverged() && icp->getFitnessScore() < sample_fitness_)
        {
            ...
            tl(1) = _tf(1,3);
            tl(2) = 0.0;
            tl = current_kf_.tf.rotation.toRotationMatrix()*tl;

            current_kf_.tf.rotation = q*current_kf_.tf.rotation;
