#include "CMScanMatcher1.h"

namespace dslam
{
CMScanMatcher1::CMScanMatcher1()
{
    // **** state variables
...
void CMScanMatcher1::configure(Configuration &config, ros::NodeHandle &nh)
{
    ...
    Configuration cocnf = config.get<Configuration>("correlative_scan_matcher", Configuration());

    // resolution of the matching grid
    double kernel_resolution = (double)cocnf.get<double>("kernel_resolution", 0.025);
    double kernel_radius = cocnf.get<double>("kernel_radius", 0.2);
    int loop_closing_window = (int)cocnf.get<double>("loop_closing_window",10);
    double max_matching_score = cocnf.get<double>("max_matching_score", 0.15);
    double inlier_threshold = cocnf.get<double>("inlier_threshold",2.0);
    int min_inlier = (int)cocnf.get<double>("min_inlier", 7);
    max_optimization_step_ = (int)cocnf.get<double>("max_optimization_step", 5);
    ...
    if (use_imu_)
        imu_subscriber_ = nh.subscribe(imu_topic_, 1, &CMScanMatcher1::imuCallback, this);
    ...
    if (use_odom_)
    {
        ...
        synchronizer_->registerCallback(boost::bind(&CMScanMatcher1::scanOdomCallBack, this, _1, _2));
    }
    else
    {
        scan_subscriber_ = nh.subscribe(scan_topic_, 1, &CMScanMatcher1::scanCallback, this);
    }
}
CMScanMatcher1::~CMScanMatcher1()
{
...
void CMScanMatcher1::imuCallback(const sensor_msgs::Imu::ConstPtr &imu_msg)
{
...
void CMScanMatcher1::odomCallback(const nav_msgs::Odometry::ConstPtr &odom_msg)
{
    boost::mutex::scoped_lock(mutex_);
...
void CMScanMatcher1::scanOdomCallBack(const sensor_msgs::LaserScanConstPtr &scan_msg, const nav_msgs::OdometryConstPtr &odom_msg)
{
    boost::mutex::scoped_lock(mutex_);
    latest_odom_msg_ = *odom_msg;
    if (!received_odom_)
...
void CMScanMatcher1::scanCallback(const sensor_msgs::LaserScan::ConstPtr &scan_msg)
{
...
void CMScanMatcher1::processScan(LDP &curr_ldp_scan, const ros::Time &time)
{
    boost::mutex::scoped_lock(mutex_);
    ...
    if(current_kf_.index == 0 || dist >= kf_dist_linear_sq_ || yaw >= kf_dist_angular_)
    {

        current_kf_.f2b = f2b_;
        addData(current_kf_, current_kf_.index == 0);
...
void CMScanMatcher1::addData(kf_t& kf, bool first)
{
    RobotLaser* laser = convertScan(kf);
    SE2 current_estimated = SE2(kf.f2b.getOrigin().x(),
                              kf.f2b.getOrigin().y(),
                              tf::getYaw(kf.f2b.getRotation()));;

    if(first)
    {
        slam_.setInitialData(current_estimated,laser);
    } 
    else
    {
        slam_.addData(current_estimated, laser);
        slam_.findConstraints();
        slam_.optimize(max_optimization_step_);
        current_estimated = slam_.lastVertex()->estimate();

        double x = current_estimated.translation().x();
        double y = current_estimated.translation().y();
        double th = current_estimated.rotation().angle();
        createTfFromXYTheta(x,y, th, current_kf_.f2b);

        fixed_to_dom_ = current_kf_.f2b*current_kf_.odom.inverse();
    }
}

RobotLaser *CMScanMatcher1::convertScan(kf_t& kf)
{
    boost::mutex::scoped_lock(mutex_);
    LaserParameters lparams(0, kf.scan.ranges.size(), kf.scan.angle_min, kf.scan.angle_increment, kf.scan.range_max, 0.1, 0); 
    lparams.laserPose = SE2(base_to_laser_.getOrigin().x(),
                              base_to_laser_.getOrigin().y(),
                              tf::getYaw(base_to_laser_.getRotation()));;

    RobotLaser *rlaser = new RobotLaser;
    rlaser->setLaserParams(lparams);

    SE2 odom = SE2(kf.odom.getOrigin().x(),
                kf.odom.getOrigin().y(),
                tf::getYaw(kf.odom.getRotation()));

    rlaser->setOdomPose(odom);
    std::vector<double> ranges(kf.scan.ranges.size());
    for (size_t i = 0; i < kf.scan.ranges.size(); i++)
    {
        //if(scan_msg->ranges[i] > max_laser_range_)
        //    ranges[i] = -1;
        //else
            ranges[i] = kf.scan.ranges[i];
    }
    rlaser->setRanges(ranges);
    rlaser->setTimestamp(kf.scan.header.stamp.sec + kf.scan.header.stamp.nsec * pow(10, -9));
...
void CMScanMatcher1::publishTf()
{
    if (!initialized_)
        return;
    //ROS_DEBUG("publishing tf %s -> %s", fixed_frame_.c_str(), odom_frame_.c_str());
    tf::StampedTransform transform_msg(fixed_to_dom_, ros::Time::now(), fixed_frame_, "odom");
    tf_broadcaster_.sendTransform(transform_msg);
}

void CMScanMatcher1::publishGraph()
{
    boost::mutex::scoped_lock(mutex_);
    if (!initialized_)
        return;
...
void CMScanMatcher1::laserScanToLDP(const sensor_msgs::LaserScan::ConstPtr &scan_msg,LDP &ldp)
{
...
void CMScanMatcher1::createCache(const sensor_msgs::LaserScan::ConstPtr &scan_msg)
{
    a_cos_.clear();
    a_sin_.clear();
...
bool CMScanMatcher1::getBaseToLaserTf(const std::string &frame_id)
{
...
void CMScanMatcher1::getPrediction(double &pr_ch_x, double &pr_ch_y,
                                double &pr_ch_a)
{
    boost::mutex::scoped_lock(mutex_);
...
void CMScanMatcher1::createTfFromXYTheta(
